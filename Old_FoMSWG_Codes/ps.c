...
/* Get mass threshold of haloes with specified number density cutoff
 * (in comoving Mpc^3 times mean density)
 * Also returns bias of objects above the mass cut
 * ntest should be NULL (only non-null for testing purposes)
 */
void get_mcut2(double n, double z, COSMOPARAM *p, double *mcut, double *bcut, double *ntest) {
  double M, n_actual_old, n_actual;
  double fM;
  double lnMstep = 0.01;
  double n_, bn_, Mc;
  ...
  /* Now that we have enough haloes, figure out where the threshold was */
  Mc = M*exp(lnMstep*(n-n_actual)/(n_actual_old-n_actual));

  n_ = bn_ = 0.;
  for(M=Mc*exp(.5*lnMstep);M<1e12*(Mc<1?1:Mc);M*=exp(lnMstep)) {
    fM = get_fM(p, M, z);
    n_ += lnMstep*fM;
    bn_ += lnMstep*fM*get_bM(p,M,z);
  }
  *bcut = bn_/n_;
  if (ntest) *ntest=n_/n; /* should be 1 */
  *mcut=Mc;
}
...
#else
  /* Use place-holder */
  for(Lc=2; Lc<=L_CMBLENS_MAX; Lc++) noiseCMB[Lc] = 1e-8;
#endif
