...
void get_bao_fisher_matrix2(double **F, char FileName[], COSMOPARAM *p, int flag) {
  ...
  for(i=0; i<N; i++) {
...
#if 1
        /* bug fix!!! Jan. 10, 2018 */
        /* T = 2/3 j_2(ks) + 1/3 - 1/3 j_0(ks)
         *   = (2/ks^3 - 2/3ks) sin ks - 2/ks^2 cos ks + 1/3 - 1/3ks sin ks
         *   = - 1/ks sin ks - 2/ks^2 (cos ks - sin ks/ks) + 1/3
         */
        ks=k*s;
        if (ks>.005) {
          T = -sin(ks)/ks - 2./ks/ks*(cos(ks)-sin(ks)/ks) + 1./3.;
        }
        else { T = ks*ks/10.;}
#endif
        var += T*D2[ik] / (k*k);
        var1 += T*D2[ik] / (k*k) / (1. + 1./nP);
      }
      rcmin = sqrt(var1/var);
    ...
    recons = sqrt((1-rcmin*rcmin)*(1-recons*recons) + recons*recons);
    ...
    //fprintf(stderr, "nP = %6.3lf recons = %5.3lf (%5.3lf min) Sigmas: T=%6.3lf R=%6.3lf S=%6.3lf Z=%6.3lf\n", nP, recons, sqrt(1-rcmin*rcmin), SigT, SigR, SigS, SigZ);
    ...
    printf("%5.3lf %12.5le %12.5le %8.6lf %8.6lf %8.5lf %8.6lf\n",
